...
void display_string(int line, char *s, char selectedSpeedDigits) {
  ...
  // Kind of redundant if we won't have more than 2 digits
  char digits[3];
  char digitNumberX = 0;
...
void display_update(int currentlySelectedRow, char selectedSpeed, int selectedStartState) {
	...
	for(i = 0; i < 4; i++) {
		DISPLAY_CHANGE_TO_COMMAND_MODE;
        // Most likely irrelevant to know about
		spi_send_recv(0x22);
		spi_send_recv(i);
    ...
    // j < 16 since each row has 16 characthers
      ...
      // Prints all the pixels of a characther.
        ...
        // Will only invert the last one or two last digits
        if(currentlySelectedRow==i && i==0 && selectedSpeed && j > 13){
          if(selectedSpeed < 10 ) {
            ...
            /* If required, change so that j>12 & case 2 has if(j==14)
              case 3:
              if spi_send_recv(~font[c*8 + k]);
            */
          }
          notInvertedRound--;
        }
        ...
        // Initial selectedSpeed as to quickly become false if false
        ...
        if(i == currentlySelectedRow && ((i == 0 && j < 9) || (i == 1 && j < 9) || ( i == 2 && j < 5))){
            spi_send_recv(~font[c*8 + k]);
            notInvertedRound--;
        }
